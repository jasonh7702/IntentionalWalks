...
sba.return.1001 <- function(df)
{
  Rmtx <- runExpMtx(df)
  df.states <- addstatevar(df)
  df.states %>% filter(EVENT_CD %in% c(4, 6)) -> df.states.sba
  
  # Mutate NEW.STATE into a new factor called NEW.STATE.FACTOR
  df.states.sba %>% mutate(NEW.STATE.FACTOR = factor(NEW.STATE, levels=state.names)) -> df.states.sba
  
  # Obtain relative frequencies (including 0s) for all 25 states
